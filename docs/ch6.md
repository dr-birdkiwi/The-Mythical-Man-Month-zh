...
假设一位项目经理已经拥有行事规范的架构师团队和大批实现人员，他如何确保每个人都能遵从、理解并落实架构师的决策？对于一个由 1000 人开发的系统，10 位架构师的小组如何维护系统概念上的完整性？在 System/360 硬件设计工作中，我们摸索出一套实现上述目标的方法，它们对软件项目同样适用。

## 书面规格说明——手册

手册，即书面规格说明，是不可或缺的工具，尽管光有文档还不够。手册是产品的外部规格说明，描述并规定了用户所见的每一个细节；同时，它也是架构师最主要的工作成果。

随着用户和实现人员反馈的增多，规格说明中难以使用或难以实现的地方不断被指出，规格说明也随之反复修订。然而对实现人员而言，修改的阶段化至关重要——进度表上必须标注带有日期的版本信息。

手册既要描述用户能看到的一切（包括所有界面），也要避免描述用户看不见的内部细节——后者属于实现人员的创作空间，不应受到限制。架构师必须为自己描述的任何特性提供一种可行的实现方案，但不应试图主导具体的实现过程。

规格说明的风格必须清晰、完整和准确。用户常常会单独提到某个定义，所以每条说明都必须重复所有的基本要素，所有文字都要相互一致。这往往使手册读起来枯燥乏味，但是精确比生动更加重要。
...
使用实现来作为一种定义的方式有一些优点。首先，所有问题可以通过试验清晰地得到答案，从来不需要争辩和商讨，回答是快捷迅速的。通过定义得出的答案，总是同所要求的一样精确和正确。但是，相对于这些优点的，是一系列可怕的缺点。实现可能更加过度地规定了外部功能。例如，无效的语法通常会产生某些结果。在拥有错误控制的系统中，它通常仅仅导致某种"无效"的指示，而不会产生其他的东西。在无错误控制的系统中，会产生各种副作用，它们可能被程序员所使用。例如，当我们着手在 System/360 上模拟 IBM 1401 时，有 30 个不同的"怪癖"——被认为是无效操作的副作用——得到广泛的应用，并被认为是定义的一部分。作为一种定义，实现体现了过多的内容：它不但描述了系统必须做什么，同时还声明了自己到底做了些什么。
...
对软件系统的架构师而言，还有一种更为优雅的方法来传播和强制定义，在建立模块间接口的语法约定（而非语义）时尤为有用。这项技术是设计被传递参数和共享存储器的声明，并要求编程实现在编译时的一些操作（PL/I 的宏或%INCLUDE）来包含这些声明。另外，如果整个接口仅仅通过符号名称进行引用，那么需要修改声明的时候，可以通过增加或插入新变量，或者重新编译受影响的程序。这种方法不需要修改程序内容。

## 会议和大会

无需多说，会议是必要的。然而，涉及数百人的大型协商会议往往需要大规模、极为正式地组织召集。因此，我们将会议分为两个层级：周例会和年度大会——实践证明这是一种非常有效的方式。

周例会每周举行半天，由全体架构师、硬件与软件实现人员代表和市场计划人员参与，由首席系统架构师主持。

会议中，任何人都可以提出问题和修改建议，但建议书通常以书面形式在会前分发。新问题一般会经过一段时间的讨论，重点在于探索创新解法，而不仅仅是得出结论。小组尝试发现解决问题的新思路，然后将部分方案交由一位或几位架构师详细记录成书面的变更建议说明书。

接着，对详细的变更建议做出决策。这一过程会经历几轮反复，实现人员和用户都会认真考量，正反两面的意见都会得到充分陈述。若达成共识，最好；若未达成，则由首席架构师拍板定案。这一过程需要时间，但最终发布的结论是正式而明确的。

周例会的决策产出迅速，让工作得以继续推进。若有人对结果极为不满，可立即向项目经理申诉，但这种情况极为罕见。

这种会议之所以卓有成效，原因如下：

1. 几个月内，相同的小组——架构师、用户和实现人员——每周碰面一次。大家对项目各方面情况都相当了解，无需额外花时间对成员进行背景介绍。
2. 小组成员见识深广、头脑敏锐，对所面对的问题有深刻的理解，且与产品紧密相关。没有人只是"顾问"的角色，人人都要承担具体责任。
3. 遇到问题时，同时在职权范围内外寻求解决方案。
4. 正式的书面建议使注意力高度集中，强制推动了决策的形成，避免了以会议纪要代替决策所带来的含糊不清。
5. 明确赋予首席架构师拍板的权力，避免了无休止的妥协和拖延。
...
System/360 的架构师具有两个空前有利的条件：充足的工作时间，拥有与实现人员相同的策略影响力。充足时间来自新技术的开发日程；而多重实现的同时开发带来了策略上的平等性。不同实现之间严格要求相互兼容，这种必要性是强制规格说明的最佳代言人。
...
显然，对于存有疑问的实现人员，应鼓励他们打电话询问相应的架构师，而不是一边自行猜测一边工作，这是一项很基本的措施。他们还需要认识到的是，上述问题的答案必须是可以告知每个人的权威性结论。

一种有用的机制是由架构师保存电话日志。日志中，他记录了每一个问题和相应的回答。每周，对若干架构师的日志进行合并，重新整理，并发布给用户和实现人员。这种机制很不正式，但非常快捷和易于理解。
